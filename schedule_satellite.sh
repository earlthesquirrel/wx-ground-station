...
SAT=$1
FREQ=$2
TLE_FILE=/home/linaro/wx-pi/weather.tle
...
while [ $END_EPOCH_DATE == `date +%D` ] || [ $END_EPOCH_DATE == `date --date="tomorrow" +%D` ]; do
  ...
  if [ $MAXELEV -ge 20 ]
    then
      FILEKEY="${SAT//" "}-${OUTDATE}"
      COMMAND="/home/linaro/wx-pi/receive_and_process_satellite.sh \"${SAT}\" $FREQ $FILEKEY $TLE_FILE $START_EPOCH $JOB_TIMER $MAXELEV $LAT_DIR"
      echo $COMMAND
      ...
      echo ${START_EPOCH},${END_EPOCH},${MAXELEV},${LAT_DIR},${SAT},"${TLE1}","${TLE2}" >>  /home/linaro/wx-pi/upcoming_passes.txt
  fi
